...
class ElectronSettings extends events.EventEmitter {
  ...
  _initConfigFilePath() {
    ...
    const configDir = this._options.configDir;
   ...
   */
  _observeUserConfig() {
    if (this.shouldAccessFileSystem() && !this._pathwatcher) {
      try {
        this._pathwatcher = pathwatcher.watch(
   ...
   * @private
   */
  _removeObserver(keyPath, handler) {
    const handlers = this._keyPathObservers.get(keyPath);

    // This key path does not exist.
    if (!handlers) {
      return;
    }

    // If no handler was specified, unbind all handlers.
    if (!handler) {
    ...
    const newHandlers = handlers.reduce((prev, curr) => {
      if (curr !== handler) {
        prev.push(curr);
      }
    ...
    // Delete the key path overserver if there are no handlers remaining.
    if (!newHandlers.length) {
      this._keyPathObservers.delete(keyPath);
      return;
    }

    this._keyPathObservers.set(keyPath, newHandlers);
  }
   ...
   * @param {string} keyPath
   * @param {Object} [options]
   * @param {Object} [options.applyDefaultValue=false]
   */
  unset(keyPath, options={}) {
    this._unsetValueAtKeyPath(keyPath);

    if (options.applyDefaultValue === true) {
      this._setDefaultsAtKeyPath(keyPath);
    }
  }
 ...
 * @readonly
 */
ElectronSettings.ConfigDir = path.join(USER_DATA_PATH, 'electron-settings');
...
ElectronSettings.Defaults = {
  configDir: ElectronSettings.ConfigDir,
